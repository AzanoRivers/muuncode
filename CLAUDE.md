...
- **Repository layout**: the frontend project lives in its own `front/` folder at the
  ...
  `vercel dev` run, which failed). `api/_lib/` holds the shared logic the functions
  import: the leading underscore is Vercel's own convention for excluding a folder
  under `api/` from function auto-discovery (without it, Vercel treats every `.ts`
  file under `api/`, including helper modules with no request handler, as its own
  serverless function, both inflating the function count against plan limits and
  breaking each helper's own isolated type-check). This is still one repository (see
  "Repository Scope"): `api/` here means "this IDE's own thin serverless glue", never
  the separate firmware-compilation project. `@types/node` is a real `dependency` in
  root `package.json`, not a `devDependency` like `typescript`/`@vercel/node`: that
  isolated per-function build only traces real imports, and `tsconfig.json`'s ambient
  `"types": ["node"]` entry has no import statement to trace, so it needs to be a real
  dependency to be present there.
...
├── vercel.json
├── .env.example
├── package.json             ← root deps for the api/ functions: @vercel/node,
│                              typescript (devDependencies); @types/node is a real
│                              dependency, not dev, see "Deployment" below
├── api/
│   ├── auth-exchange.ts
│   ├── auth-refresh.ts
│   └── _lib/
